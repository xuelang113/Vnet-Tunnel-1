...
check_sys_clinet(){
	suidaoanquan
    wget -N --no-check-certificate "https://blog.liukersun.top/wp-content/uploads/2021/03/tunnel.zip" 
	unzip tunnel.zip
	chmod -R +x ./*
...
install_server(){
	suidaoanquan
    wget -N --no-check-certificate "https://blog.liukersun.top/wp-content/uploads/2021/03/tunnel.zip" && unzip tunnel.zip && chmod -R +x ./*
...
chongqi_server(){
	...
	nohup ./server >> /dev/null 2>&1 &
	echo -e ${Green_font_prefix}"重启完成-请重新添加配置"${Font_color_suffix}
	echo -e ${Green_font_prefix}"http://${SERVER_IP}:8081/resources/add_client.html"${Font_color_suffix}
}
